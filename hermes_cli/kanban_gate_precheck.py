...
Enforcement point: ``hermes_cli/kanban_db_dispatch.py``'s F1 choke point,
gated by the per-board config flag ``kanban.gate_precheck_enabled``
(default ``False`` — opt-in, unlike F1's default-gated batch-approval check;
this is a new, unproven heuristic and the risk profile is materially
different, so a board must explicitly turn it on).
"""
