...
| `on_kanban_task_created` | Observer | After task creation commits; return ignored. | `task_id`, `profile_name`, `board`, `assignee`, `run_id`, `project_id`, `workspace_kind` | Board/task/profile identifiers and project/workspace metadata. |
...
| `kanban_task_completed` | Observer | After completion and cleanup, usually in worker process; archived tasks use `outcome="archived"` and normal completions use `outcome="completed"`; return ignored. | `task_id`, `profile_name`, `board`, `assignee`, `run_id`, `summary`, `outcome` | Summary may contain project/user content. |
...
All four kanban lifecycle hooks are observer-only and carry `task_id`, `profile_name`, `board`, `assignee`, and `run_id`; created adds `project_id` and `workspace_kind`, completed adds `summary` and `outcome`, and blocked adds `reason`.

### Kanban worker-lifecycle, task-mutation, and dispatch observers

Six additional observers (RFC #58548) extend the kanban family. All are observer-only, fire after the relevant transaction commits, and short-circuit on `has_hook` — with no subscriber, dispatch behavior is unchanged. Task-scoped hooks carry the same common fields as the hooks above.

- **`on_kanban_task_created`** — after task creation commits. Adds `project_id` and `workspace_kind`.
...
- **`on_kanban_worker_exited`** — tick-derived, when `detect_crashed_workers` reclaims a dead-PID task or `enforce_max_runtime` times out a task. Adds `worker_pid`, `exit_kind`, `exit_code`, `outcome`, `retry_status`.
