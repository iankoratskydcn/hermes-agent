...
Owner choice under test (documented in kanban_decompose.py and the
Rule 6 commit message): when the aux LLM cannot EARS-ify a child task
without inventing missing details, the decomposer does NOT block task
creation — it files a decision-hud problem report via
...
Two behavior contracts:
  1. success path — a real, template-conformant EARS sentence from the
     LLM is stored verbatim, and no problem report is filed.
  2. failure path — a refusal (or an LLM-claimed sentence that fails
     the mechanical EARS-shape check) results in ears_sentence staying
     None, and exactly one problem report being filed; the code never
     invents a plausible-looking sentence to paper over the gap.
"""
